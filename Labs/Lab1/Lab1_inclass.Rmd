...
```{r exploration-ate-att, class.source='details hide', results.folding='hide', summary.source = 'ATEATT.R', summary.results = 'Answer'}
# Check the composition of our experimental sample
exp_group %>% 
  group_by(job, treated) %>% 
  summarise(n = n())

# Simple difference-in-means (ATE estimate)
t_res <- t.test(new_sal ~ treated, data = exp_group)
ate <- t_res$estimate[2] - t_res$estimate[1]
ate

# Explanation of negative estimated ATE
exp_group %>% 
  group_by(job, treated) %>% 
  summarise(eff_size = mean(new_sal))

exp_group %>% 
  group_by(treated) %>% 
  summarise(eff_size = mean(new_sal))

# Constructing a more appropriate control group (bankers only) for the ATT
control_banker <- workers[-treated_rows,] %>% filter(job == 'banker')
banker_control_sal <- control_banker$salary

exp_group2 <- bind_rows(treated, control_banker)
exp_group2$new_sal <- c(treat_eff, banker_control_sal) 
exp_group2$treated <- c(rep(1, 500), rep(0, 500)) 

t_res2 <- t.test(new_sal ~ treated, data = exp_group2)
att <- t_res2$estimate[2] - t_res2$estimate[1]
att
```

- We see that the naive ATE is negative, which suggests the training is **hurting** participants on average. However, that's an artifact of mixing high-salary doctors into the control group.  
- Once we limit the control group to bankers only, we get a **positive** effect, which reflects the **ATT** for the subgroup that would actually participate in training.
...
```{r bias-decomposition, class.source='details hide', results.folding='hide', summary.source = 'BiasDecomp.R', summary.results = 'Answer'}
# Selection bias
...
Here, we see how the difference-in-means **contains** the true ATE *plus* two bias terms:

1. **Selection Bias**: Occurs when treatment assignment is systematically related to potential outcomes.  
2. **Heterogeneous Treatment Effects**: If some benefit more than others, and those “better-off” folks are more (or less) likely to be treated, it skews the overall difference-in-means.
...
```{r random-assignment-large, class.source='details hide', results.folding='hide', summary.source = 'RandomLarge.R', summary.results = 'Answer'}
set.seed(123)
po_patients_big <- tibble(
  patients = c(1:1000),
  y_i1 = rnorm(1000, .6, .5), # Treatment ~ N(0.6, 0.5^2)
  y_i0 = rnorm(1000, 0, .5),  # Control   ~ N(0,   0.5^2)
  po_diff = y_i1 - y_i0
) %>% 
  mutate(
    D = rbinom(1000, 1, .5),
...
# ATT and ATC (should be close to ATE in large random sample)
...
# Check selection and HTE biases
...
With 1,000 patients, randomization becomes even more powerful. We see that the **difference in means** closely approximates the **true ATE**, and biases effectively drop to zero.
